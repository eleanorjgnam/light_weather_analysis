...
## Helper Functions
```{r}
nightdater <- function (colIn) { ## Function to round early-morning dates/datetimes back to the previous night, to create a "night-of" date. For example, a bird tagged at 23:00 on July 15 and a bird tagged at 03:00 on July 16 would both be considered tagged on the "night of" July 15. 
...
```{r}
baccalieu_weather <- read_csv("data/baccalieu_2025_weather.csv") %>% 
...
gull_weather <- read_csv("data/gull_2025_weather.csv") %>% 
  mutate(datetime = as_datetime(Time, format = "%m/%d/%Y %H:%M")) %>% 
  mutate(datetime = force_tz(datetime, tzone = "America/St_Johns")) %>% 
  filter(datetime > "2025-06-16" & datetime < "2025-10-29") %>% 
  mutate(`Wind Speed` = NA) ## This impeller never worked

great_weather <- read_csv("data/great_2025_weather.csv") %>% 
  mutate(datetime = as_datetime(Time, format = "%m/%d/%Y %H:%M")) %>% 
  mutate(datetime = force_tz(datetime, tzone = "America/St_Johns")) %>% 
...
```{r}
baccalieu_light <- read_csv("data/baccalieu_light_20251023.csv")
